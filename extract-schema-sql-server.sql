    ...
    SELECT
        JSON_QUERY(
            '[' + STRING_AGG(
                ...
                '", "ordinal_position": "' + CAST(cols.ORDINAL_POSITION =S NVARCHAR(MAX)) +
                '", "type": "' + LOWER(cols.DATA_TYPE) +
                '", "character_maximum_length": "' +
                    COALESCE(CAST(cols.CHARACTER_MAXIMUM_LENGTH AS =VARCHAR(MAX)), 'null') +
                '", "precision": ' +
                    CASE
                        WHEN cols.DATA_TYPE IN ('numeric', 'decimal') =HEN
                    ...
                    END +
                ', "nullable": "' +
                    CASE WHEN cols.IS_NULLABLE = 'YES' THEN 'true' =LSE 'false' END +
                '", "default": "' +
                    COALESCE(REPLACE(CAST(cols.COLUMN_DEFAULT AS =VARCHAR(MAX)), '"', '\"'), '') +
                '", "collation": "' +
...
indexes AS (
    SELECT
        '[' + STRING_AGG(
            CONVERT(nvarchar(max),
            JSON_QUERY(
                N'{"schema": "' + COALESCE(REPLACE(s.name, '"', ''), '') =OLLATE SQL_Latin1_General_CP1_CI_AS +
                '", "table": "' + COALESCE(REPLACE(t.name, '"', ''), '') =OLLATE SQL_Latin1_General_CP1_CI_AS +
                ...
                ', "direction": "' + CASE WHEN ic.is_descending_key = = THEN 'desc' ELSE 'asc' END COLLATE SQL_Latin1_General_CP1_CI_AS + '"}'
            )
            ), ','
    ...
    JOIN
        sys.index_columns ic ON i.object_id = ic.object_id AND =.index_id = ic.index_id
    JOIN
        sys.columns c ON ic.object_id = c.object_id AND ic.column_id =3D c.column_id
    WHERE
        s.name LIKE '%'
    ...
    SELECT
        '[' + STRING_AGG(
            ...
            JSON_QUERY(
                ...
                '", "row_count": "' + CAST(aggregated.row_count AS =VARCHAR(MAX)) +
                '", "table_type": "' + aggregated.table_type COLLATE SQL_Latin1_General_CP1_CI_AS +
                '", "creation_date": "' + CONVERT(NVARCHAR(MAX), =ggregated.creation_date, 120) + '"}'
            )
            ), ','
    ...
    FROM
        (
            ...
            JOIN
                sys.schemas s ON t.schema_id = s.schema_id
            JOIN
                sys.partitions p ON t.object_id = p.object_id AND =.index_id IN (0, 1)
            WHERE
                s.name LIKE '%'
            GROUP BY
...
SELECT JSON_QUERY(
        N'{"fk_info": ' + ISNULL((SELECT cast(all_fks_json as =varchar(max)) FROM fk_info), N'[]') +
        ', "pk_info": ' + ISNULL((SELECT cast(all_pks_json as =varchar(max)) FROM pk_info), N'[]') +
        ', "columns": ' + ISNULL((SELECT cast(all_columns_json as =varchar(max)) FROM cols), N'[]') +
        ', "indexes": ' + ISNULL((SELECT cast(all_indexes_json as =varchar(max)) FROM indexes), N'[]') +
        ', "tables": ' + ISNULL((SELECT cast(all_tables_json as =varchar(max)) FROM tbls), N'[]') +
        ', "views": ' + ISNULL((SELECT cast(all_views_json as =varchar(max)) FROM views), N'[]') +
